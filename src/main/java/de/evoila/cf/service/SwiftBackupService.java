...
public abstract class SwiftBackupService extends AbstractBackupService{
    ...
    protected String upload(File backup, DatabaseCredential source, FileDestination destination, BackupJob job) throws IOException, OSException {
        ...
        log.info(String.format("Uploading Backup (%s)", job.getId()));
        ...
        log.info(String.format("Uploading the Backup (%s) from %s:%d/%s took %fs (File size %f)",
                               job.getId(),
                               source.getHostname(),
                               source.getPort(),
                               source.getContext(),
    ...
    public File download (FileDestination source, BackupJob job) throws IOException, OSException {
        long s_time = System.currentTimeMillis();
        log.info(String.format("Downloading Backup (%s)", job.getId()));
        ...
        log.info(String.format("Download (%s) of File %s/%s took %f s (File size %f)",
                               job.getId(),
