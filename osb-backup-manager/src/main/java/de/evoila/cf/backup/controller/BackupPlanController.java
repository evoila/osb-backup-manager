 ...
 */
@Tag(name = "/backupPlans", description = "Manage individual BackupPlans or BackupPlans grouped by service instances.")
@Controller
public class BackupPlanController {
    ...
    @Operation(summary = "Get all BackupPlan from the specified service instance.")
    @GetMapping("/backupPlans/byInstance/{serviceInstanceId}")
    ...
    @Operation(summary = """
            Save the BackupPlan in the repository and add it as a task in the scheduling service if \
            not paused. The task will periodically add backup jobs to the queue, as is configured in the BackupPlan.\
            """)
    @PostMapping("/backupPlans")
    ...
    @Operation(summary = "Get the BackupPlan from the repository specified by its ID.")
    @GetMapping("/backupPlans/{planId}")
    public ResponseEntity<BackupPlan> get(@PathVariable() ObjectId planId) {
        BackupPlan response = backupPlanService.getPlan(planId);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @Operation(summary = "Delete the BackupPlan and running task from the repository and scheduling service.")
    @DeleteMapping("/backupPlans/{planId}")
    ...
    @Operation(summary = "Delete all BackupPlans, running tasks from the repository and scheduling service.")
    @DeleteMapping("/backupPlans/byInstance/{serviceInstanceId}")
    ...
    @Operation(summary = """
            Update BackupPlan, running task and deletes old backup files & jobs exceeding the \
            retention period specified in the BackupPlan.\
            """)
    @PatchMapping("/backupPlans/{planId}")
